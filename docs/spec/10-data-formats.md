...
スナップショットの `troop_type` は人物ポリゴンの兵科別配色・装備種別indexとして
使う。位置、向き、状態、騎乗、転倒はシミュレーションが正本で、体格差、モーション
位相、切替クロスフェード、16ボーン姿勢、LOD形状は `sim-render` が保持する。
人物画像、スプライトセット、実行時manifestは使用しない。
